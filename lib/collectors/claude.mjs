...
function scanTranscriptUsage(filePath, priorSeen = new Set()) {
  ...
  for (const line of fs.readFileSync(filePath, 'utf8').split('\n')) {
    ...
    if (!messageId || !usage || transcriptSeen.has(messageId)) continue;

    transcriptSeen.add(messageId);
...
/** Current transcript watermark captured before a new turn starts. */
export function collectClaudeWatermark({ harnessDir }) {
  const chosenPath = findLatestTranscript(harnessDir);
  if (!chosenPath || !fs.existsSync(chosenPath)) {
    return { ok: false, reason: `No Claude transcript found for ${path.resolve(harnessDir)}` };
  }

  const parsed = scanTranscriptUsage(chosenPath, new Set());
  if (!parsed) {
